...
def load_data():
    ...
    GeoJSON files are already in WGS84, but we still force CRS to EPSG:4326.
    """
    ...
    areas = gpd.read_file(FINAL_AREAS_PATH)
    ...
    if markers.crs is None:
        markers = markers.set_crs(epsg=4326)

    markers = markers.to_crs(epsg=4326)

    return areas, grid, markers


try:
    areas, grid, markers = load_data()
...
def add_legend_to_map(map_object, title, color_dict):
    """
    Adds a readable custom legend to a Folium map.
    """
...
def simplify_for_map(gdf, tolerance=0.00003):
    """
    Simplifies geometries for faster Folium rendering.
    Works in EPSG:4326, so tolerance is in degrees.
    """
    if gdf.empty:
        return gdf
...
def make_downloadable_table(df):
...
st.info(
    """
    This prototype uses historical data as a risk baseline. In operational use,
...
show_markers = st.sidebar.checkbox(
    "Show operator markers on local map",
    value=False
)
...
if page == "Overview":
    ...
    areas_map = simplify_for_map(areas, tolerance=0.00005)

    overview_center = get_map_center(areas_map)
    ...
    area_popup_fields = [f for f in area_popup_fields if f in areas_map.columns]
    ...
    st_folium(overview_map, width=None, height=580)


# ============================================================
# PAGE 2 — LOCAL READINESS MAP
# ============================================================

elif page == "Local Readiness Map":
    ...
    if selected_grid.empty:
        ...
        selected_grid_map = simplify_for_map(selected_grid, tolerance=0.00003)
        selected_area_polygon_map = simplify_for_map(selected_area_polygon, tolerance=0.00005)

        map_center = get_map_center(selected_grid_map)
        ...
        if not selected_area_polygon_map.empty:
            folium.GeoJson(
                selected_area_polygon_map,
        ...
        grid_popup_fields = [
            field for field in grid_popup_fields if field in selected_grid_map.columns
        ]
        ...
        if show_markers and not selected_markers.empty:
            # Limit markers just in case a future file contains many more points.
            marker_limit = 1000
            selected_markers_limited = selected_markers.head(marker_limit)

            for _, row in selected_markers_limited.iterrows():
...
# ============================================================
...
elif page == "Priority Action List":
    ...
    if selected_grid.empty:
        ...
        csv = make_downloadable_table(action_table)
...
# ============================================================
    ...
    st.markdown(
        ...
        1. Start with the **Overview** page to see which Amsterdam project areas have higher
           overdrive-risk.
